...
(function(global){
  ...
  console.log('MazeGen: generator.js v12 loaded');
  ...
  function carveMaze(w,h,rng){
    ...
    function carve(x,y){
      grid[y][x] = 0;
      // Ensure we always have a concrete Array for iteration
      ...
      for (let i=0; i<dirs.length; i++){
        ...
        if (ny>0 && ny<h-1 && nx>0 && nx<w-1 && grid[ny][nx]===1){
          const mx = (x + (dx/2))|0, my = (y + (dy/2))|0;
          if (inBounds(mx,my,w,h)) grid[my][mx] = 0;
          carve(nx,ny);
        }
      }
    }
  ...
  function addLoops(grid, loops, rng){
    // Defensive: validate grid shape
    if(!Array.isArray(grid) || grid.length===0 || !Array.isArray(grid[0])) return 0;
    const h = grid.length, w = grid[0].length;
    let added = 0, attempts = 0;
    const maxAttempts = Math.max(loops*40, 200);
    const isWall  = (x,y)=> inBounds(x,y,w,h) && grid[y] && grid[y][x]===1;
    const isFloor = (x,y)=> inBounds(x,y,w,h) && grid[y] && grid[y][x]===0;
    while(added<loops && attempts<maxAttempts){
      attempts++;
      // choose interior cell; odds-only not required if we verify adjacency later
      const x = 1 + ((rng()* (w-2))|0);
      const y = 1 + ((rng()* (h-2))|0);
  ...
  function farthestFrom(grid, sx, sy){
    ...
    let best=[sx,sy,0];
    while(q.length){ const [x,y,d]=q.shift(); if(d>best[2]) best=[x,y,d];
  ...
  function generateLevel(level, seed){
    ...
    try {
      ...
      console.warn('addLoops failed; continuing without loops:', e);
    }
