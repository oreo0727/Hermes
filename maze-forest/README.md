...
Sprite input rules
- A labeled art board, presentation sheet, screenshot, or mood/reference composite is not a runtime-ready sprite sheet.
- The legacy `assets/sheets/user-pack.png` fallback only accepts a true 6x6 uniform grid at 128 px per cell (768x768 total). Anything else is rejected as the wrong production format.
- For production integration, use one of:
  - loose transparent PNG sprites
  - a true uniform grid sheet with known cell geometry
  - an atlas PNG plus JSON manifest
